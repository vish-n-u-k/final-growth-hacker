  ...
  return (
    <div className="st-section">
      <div className="st-section-hd">
        <h2 className="st-section-title">Sales Playbook</h2>
...
function IntegrationsSection({
  ...
  const oauthProviders = ['meta_oauth', 'instagram_oauth', 'linkedin_oauth', 'pinterest_oauth']
...
const OAUTH_PLATFORM_META: Record<string, { color: string; label: string; icon: React.ReactNode }> = {
  meta_oauth: {
    color: '#1877f2',
    label: 'Connect with Meta (Facebook)',
    icon: (
...
function OAuthCard({
  ...
  } else if (def.provider === 'instagram_oauth') {
    if (m.instagram_username) displayLines.push(`@${m.instagram_username}`)
    if (m.instagram_followers) displayLines.push(`${Number(m.instagram_followers).toLocaleString()} followers`)
    if (m.instagram_posts) displayLines.push(`${m.instagram_posts} posts`)
