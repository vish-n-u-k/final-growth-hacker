          ...
          items: [
            ...
            },
            {
              ...
              fixGuide: [
                ...
                'Copy your Project API Key (starts with phc_) from PostHog → Project Settings → Project API Key',
                'Open `md_files/posthog-setup.md` from your project root and paste its full contents to Claude Code — it contains step-by-step install instructions plus every event the app should track (auth, onboarding, analysis, item checks, exports, AI features, score milestones)',
                'Call posthog.identify(userId, { email, name }) immediately after a user signs up or logs in — this links all events to a Person record',
                'To connect PostHog to this dashboard: PostHog → Settings → Personal API keys → Create personal API key (phx_...) → note your numeric Project ID from the URL → save both in Settings → Integrations → PostHog',
              ],
            },
          ],
        },
