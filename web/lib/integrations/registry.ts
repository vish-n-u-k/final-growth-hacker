...
export const INTEGRATION_REGISTRY: IntegrationDefinition[] = [
  ...
  },
  {
    ...
    description: 'Pulls live GA4 metrics (sessions, users, pageviews) into your analytics dashboard. Uses a Service Account key — no OAuth flow needed.',
    type: 'api_key',
    group: 'analytics',
    ...
    fields: [
      {
        ...
        inputType: 'text',
        helpText: 'Google Cloud Console → IAM & Admin → Service Accounts → create account → create JSON key → copy client_email. Then add this email as a Viewer in GA4 → Admin → Property Access Management.',
        isMetadata: true,
      },
      {
        key: 'private_key',
      ...
      },
      {
        ...
        inputType: 'text',
        helpText: 'Numeric ID only. Found in GA4 → Admin → Property Settings → Property ID.',
        isMetadata: true,
      },
    ],
  },
  {
    ...
    description: 'Record how you verified your site in GSC. Fill in whichever method you used — the agent will detect it automatically.',
    type: 'api_key',
    group: 'analytics',
    ...
    fields: [
      {
        ...
        inputType: 'text',
        helpText: 'HTML tag method: in GSC → Add Property → HTML tag → copy only the content="..." value (not the whole tag).',
        isMetadata: true,
        optional: true,
      },
      {
        key: 'gsc_html_filename',
        label: 'HTML File Name',
        placeholder: 'google1a2b3c4d.html',
        inputType: 'text',
        helpText: 'HTML file method: GSC gives you a filename to upload to your site root. Paste just the filename here (e.g. google1a2b3c4d.html).',
        isMetadata: true,
        optional: true,
      },
      {
        key: 'gsc_dns_txt_value',
        label: 'DNS TXT Value',
        placeholder: 'google-site-verification=XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX',
        inputType: 'text',
        helpText: 'DNS TXT record method: paste the full value Google gave you to add as a TXT record on your domain\'s @ entry.',
        isMetadata: true,
        optional: true,
      },
    ],
  ...
  },
  {
    provider: 'google_ads',
    name: 'Google Ads',
    description: 'Pull keyword ideas from the Google Ads Keyword Planner — real search volumes and competition data.',
    type: 'api_key',
    group: 'analytics',
    docsUrl: 'https://developers.google.com/google-ads/api/docs/start',
    setupSteps: [
      'Go to [Google Cloud Console](https://console.cloud.google.com) → APIs & Services → Library → enable the "Google Ads API"',
      'Create OAuth 2.0 credentials: APIs & Services → Credentials → Create Credentials → OAuth client ID → Web application → add `https://developers.google.com/oauthplayground` as an authorised redirect URI → copy Client ID and Client Secret',
      'Get a Developer Token: sign in to [Google Ads](https://ads.google.com) → Tools → API Centre → apply for Basic access (or use test token for testing)',
      'Get a Refresh Token via [OAuth Playground](https://developers.google.com/oauthplayground): click the gear icon → check "Use your own OAuth credentials" → enter Client ID and Secret → in Step 1 select "Google Ads API v17" scope → authorise → in Step 2 click "Exchange authorisation code for tokens" → copy the Refresh Token',
      'Your Customer ID is the numeric ID shown in the top-right of Google Ads (e.g. 123-456-7890 — enter without dashes)',
    ],
    fields: [
      {
        key: 'developer_token',
        ...
        inputType: 'password',
        helpText: 'Google Ads → Tools & Settings → API Centre → Developer Token.',
        isMetadata: true,
      },
      {
        key: 'client_id',
        ...
        inputType: 'text',
        helpText: 'Google Cloud Console → APIs & Services → Credentials → your OAuth 2.0 Client ID.',
        isMetadata: true,
      },
      {
        key: 'client_secret',
        ...
        inputType: 'password',
        helpText: 'Found alongside the Client ID in Google Cloud Console.',
        isMetadata: true,
      },
      {
        key: 'refresh_token',
        label: 'Refresh Token',
        placeholder: '1//0gxxxxxxxxxxxxxxxxxxxxxxxx',
        inputType: 'password',
        helpText: 'Obtained from OAuth Playground — see setup steps above.',
        isMetadata: true,
      },
      {
        key: 'customer_id',
        ...
        inputType: 'text',
        helpText: 'Numeric ID shown in Google Ads top-right. Enter digits only, no dashes.',
        isMetadata: true,
      },
    ],
  },
