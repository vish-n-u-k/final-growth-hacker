...
import { brands, modules, moduleCategories, moduleItems, reminders } from '@/lib/db/schema'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Seed reminder presets based on website type
    try {
      const presets = getPresetsForWebsiteType(websiteType ?? null)
      const now = new Date()
      if (presets.length > 0) {
